...
public class QLearningSwordmasterController : AISwordmasterController
{
    ...
    private string GetState()
    {
        ...
        bool canLunge = Time.time >= lastLungeTime + lungeCooldown;
        bool canFatalFlurry = Time.time >= lastFatalFlurryTime + fatalFlurryCooldown;
        bool canPoison = Time.time >= lastPoisonTime + poisonCooldown;
        bool canAttack = Time.time >= lastAttackTime + attackCooldown;
        bool inAttackRange = distance <= attackRange;
        bool bossLowHealth = bossHealthPercentage <= fatalFlurryThreshold;

        return $"dist_{Mathf.Round(distance)}_hp_{Mathf.Round(healthPercentage * 10)}_" +
               $"bossHp_{Mathf.Round(bossHealthPercentage * 10)}_" +
               $"lunge_{canLunge}_flurry_{canFatalFlurry}_poison_{canPoison}_" +
               $"atk_{canAttack}_inAtk_{inAttackRange}_bossLow_{bossLowHealth}";
    }
    ...
    private void MoveTowardBoss()
    {
        ...
        movement.Move(direction * swordmasterSpeed);
    }
    ...
    private float CalculateReward()
    {
        ...
        switch (lastAction)
        {
            case 0 when Time.time - lastLungeTime < 0.1f: // Lunge
                reward += 5f;
                break;
            case 1 when Time.time - lastFatalFlurryTime < 0.1f: // Fatal Flurry
                reward += 15f;
                break;
            case 2 when Time.time - lastPoisonTime < 0.1f: // Poison
                reward += 8f;
                break;
    ...
    {
        return Path.Combine(Application.persistentDataPath, "qlearning_swordmaster_save.json");
    }
