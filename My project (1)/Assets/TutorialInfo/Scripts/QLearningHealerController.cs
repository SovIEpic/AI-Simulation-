...
public class QLearningHealerController : AIHealerController
{
    ...
    private string GetState()
    {
        ...
        bool canSiphon = Time.time >= lastSiphonTime + siphonCooldown;
    ...
    private void ExecuteAction(int action)
    {
        ...
        {
            case 0: // Resus
                if (canResus && HasDeadTeammate())
                {
                    TryResus();
                }
                break;
            case 1: // Siphon
                if (!siphonActive && Time.time >= lastSiphonTime + siphonCooldown)
                {
                    TrySiphon();
                }
                break;
    ...
    public void SetResusUsed() => canResus = false;
    ...
    private float CalculateReward()
    {
        ...
        switch (lastAction)
        {
            case 0 when Time.time - lastResusTime < 0.1f: // Resus
                reward += 20f;
                break;
    ...
    {
        return Path.Combine(Application.persistentDataPath, "qlearning_healer_save_v2.json");
    }
